...
var logger = jm.getLogger('jm-ms-daorouter:router');
var ERR = jm.ERR;

module.exports = function(dao, opts) {
    var router = jm.ms(opts);
    jm.enableEvent(dao);
     ...
     * }
     */
    router.useField = function(opts, cb) {
        if(!opts || !opts.field) {
            var doc = ERR.FA_PARAMS;
            var err = new Error(doc.msg);
            if (!cb)
                throw err;
            else
                cb(err, doc);
        }else{
            var array = require('./router_array.js');
            var prefix = opts.prefix || opts.field;
            this.use({uri: '/:pid/' + prefix, fn: array(dao, opts)}, cb);
        }
        return this;
    };
